...
public class Rhombus extends ClosedFigure {
	...
	protected void doPaint(Graphics2D g) {
		...
		Point n = bbox.getPosControlPoint(Cardinal.N);
		Point s = bbox.getPosControlPoint(Cardinal.S);
		Point w = bbox.getPosControlPoint(Cardinal.W);
		Point e = bbox.getPosControlPoint(Cardinal.E);

		final int diff = BoundBox.getSizeControlPoint() / 2;

		int[] x = { n.x + diff, w.x + diff, s.x + diff, e.x + diff };
		int[] y = { n.y + diff, w.y + diff, s.y + diff, e.y + diff };
